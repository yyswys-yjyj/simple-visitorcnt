...
|管理面板|提供便捷的管理方法，2fa+cookie；高隐蔽性，未授权访问一律404|
|轻量级|200多行代码|
...
## 功能解释
### 1.manager是怎么防止未授权访问的
在2FA配置完成后，除非输入了正确的一次性密码，否则无论如何尝试都会返回404    
cookie是动态生成的，当登录完成后，会随机生成一串128位的字符串作为token，只有持有该token的管理员访问manager，才能进入管理面板，否则一律返回404   

## 坑
登录状态依赖一个在json中叫**全局锁定开关**的字段（lock），当状态为`false`时，则将允许创建新路由（即访问不存在的路由时自动创建），请尽快在管理面板中完成操作并及时锁定，否则计数器可能会被滥用
